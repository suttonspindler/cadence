...
export function SiteHeader({ user }: { user: SessionUser }) {
  ...
  return (
    <header className="bg-paper-raised/70 backdrop-blur">
          ...
          <div className="hidden items-center gap-4 md:flex">
            <form action="/find" method="get" role="search" className="relative">
              ...
              <input
                ...
                className="w-56 rounded-full border border-line bg-paper py-2 pl-9 pr-3.5 text-sm text-ink outline-none transition focus:border-accent-soft lg:w-72"
              />
            </form>
            ...
            {user ? (
              ...
              >
                <button
                  ...
                  className="flex items-center gap-2 rounded-full py-0.5 pl-0.5 pr-2 text-sm text-ink-soft transition hover:text-accent"
                >
              ...
              <Link
                href="/signin"
                className="rounded-full bg-accent px-3 py-1.5 text-sm text-paper-raised transition hover:bg-accent-soft"
              >
                Sign in
              </Link>
            )}
      ...
      {/* Tier 2 — browse nav (desktop) */}
      <div className="hidden border-b border-line md:block">
        <nav className="mx-auto flex max-w-5xl items-center gap-6 px-6 py-2.5 text-sm text-ink-soft">
          {NAV_LINKS.map((l) => (
      ...
      {/* Mobile menu panel */}
      {open && (
        <nav className="border-b border-line md:hidden">
